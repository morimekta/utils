...
public class Diff {
    ...
    public static class Options {
         ...
         * @return The options.
         */
        public Options withEditCost(int editCost) {
            this.editCost = editCost;
            return this;
        }

        public Options withChangeLines(boolean changeLines) {
    ...
    public Diff(String text1, String text2) {
        this(text1, text2, Options.defaults());
    }

    public Diff(String text1, String text2, Options options) {
        ...
        this.deadline = getDeadline(options);

        this.changeList = main(text1, text2, options.getCheckLines());
    }

    Diff(LinkedList<Change> changeList) {
        this(changeList, Options.defaults());
    }

    Diff(LinkedList<Change> changeList, Options options) {
        this(changeList, options, options.getClock().millis());
    }

    Diff(LinkedList<Change> changeList, Options options, long deadline) {
        this.options = options;
    ...
    public String prettyHtml() {
        ...
        for (Diff.Change aDiff : changeList) {
    ...
    public static Diff fromDelta(String text1, String delta)
        ...
        LinkedList<Diff.Change> diffs = new LinkedList<>();
        ...
        for (String token : tokens) {
            ...
            switch (token.charAt(0)) {
                case '+':
                    ...
                    diffs.add(new Diff.Change(Diff.Operation.INSERT, param));
                    break;
                case '-':
                    // Fall through.
                case '=':
                    ...
                    if (token.charAt(0) == '=') {
                        diffs.add(new Diff.Change(Diff.Operation.EQUAL, text));
                    } else {
                        diffs.add(new Diff.Change(Diff.Operation.DELETE, text));
                    }
                    break;
                default:
        ...
        return new Diff(diffs, Options.defaults());
    }
    ...
    private LinkedList<Change> main(String text1, String text2, boolean checkLines) {
        LinkedList<Change> diffs;
        if (text1.equals(text2)) {
            diffs = new LinkedList<>();
            if (text1.length() != 0) {
                diffs.add(new Diff.Change(Diff.Operation.EQUAL, text1));
            }
            return diffs;
        }
        ...
        if (commonprefix.length() != 0) {
            diffs.addFirst(new Diff.Change(Diff.Operation.EQUAL, commonprefix));
        }
        if (commonsuffix.length() != 0) {
            diffs.addLast(new Diff.Change(Diff.Operation.EQUAL, commonsuffix));
        }
     ...
     */
    private LinkedList<Diff.Change> compute(String text1, String text2, boolean checkLines) {
        LinkedList<Diff.Change> diffs = new LinkedList<>();

        if (text1.length() == 0) {
            // Just add some text (speedup).
            diffs.add(new Diff.Change(Diff.Operation.INSERT, text2));
            return diffs;
        }

        if (text2.length() == 0) {
            // Just delete some text (speedup).
            diffs.add(new Diff.Change(Diff.Operation.DELETE, text1));
            return diffs;
        }
        ...
        if (i != -1) {
            ...
            Diff.Operation op = (text1.length() > text2.length()) ?
                                Diff.Operation.DELETE : Diff.Operation.INSERT;
            diffs.add(new Diff.Change(op, longtext.substring(0, i)));
            diffs.add(new Diff.Change(Diff.Operation.EQUAL, shorttext));
            diffs.add(new Diff.Change(op, longtext.substring(i + shorttext.length())));
            return diffs;
        }
        ...
        if (hm != null) {
            ...
            LinkedList<Diff.Change> diffs_a = main(text1_a, text2_a, checkLines);
            LinkedList<Diff.Change> diffs_b = main(text1_b, text2_b, checkLines);
            // Merge the results.
            diffs = diffs_a;
            diffs.add(new Diff.Change(Diff.Operation.EQUAL, mid_common));
     ...
     * @return Linked List of Diff objects.
     */
    private LinkedList<Diff.Change> lineMode(String text1, String text2) {
        ...
        LinkedList<Diff.Change> diffs = main(text1, text2, false);
        ...
        diffs.add(new Diff.Change(Diff.Operation.EQUAL, ""));
        int count_delete = 0;
        int count_insert = 0;
        String text_delete = "";
        String text_insert = "";
        ListIterator<Change> pointer = diffs.listIterator();
        Diff.Change thisDiff = pointer.next();
        while (thisDiff != null) {
     ...
     */
    LinkedList<Diff.Change> bisect(String text1, String text2) {
        ...
        LinkedList<Diff.Change> diffs = new LinkedList<Diff.Change>();
        diffs.add(new Diff.Change(Diff.Operation.DELETE, text1));
        diffs.add(new Diff.Change(Diff.Operation.INSERT, text2));
        return diffs;
    }
     ...
     * @return LinkedList of Diff objects.
     */
    private LinkedList<Diff.Change> bisectSplit(String text1, String text2,
                                                int x, int y) {
        ...
        LinkedList<Diff.Change> diffs = main(text1a, text2a, false);
        LinkedList<Diff.Change> diffsb = main(text1b, text2b, false);
     ...
     * @param lineArray List of unique strings.
     */
    void charsToLines(LinkedList<Diff.Change> diffs,
                                     List<String> lineArray) {
        StringBuilder text;
        for (Diff.Change diff : diffs) {
     ...
     * @param diffs LinkedList of Diff objects.
     */
    void cleanupSemantic(LinkedList<Diff.Change> diffs) {
        if (diffs.isEmpty()) {
            return;
        }
        boolean changes = false;
        Stack<Change> equalities = new Stack<Diff.Change>();  // Stack of qualities.
        String lastequality = null; // Always equal to equalities.lastElement().text
        ListIterator<Diff.Change> pointer = diffs.listIterator();
        ...
        while (thisDiff != null) {
            if (thisDiff.operation == Diff.Operation.EQUAL) {
                ...
                if (lastequality != null && (lastequality.length()
                    ...
                    pointer.set(new Diff.Change(Diff.Operation.DELETE, lastequality));
                    // Insert a corresponding an insert.
                    pointer.add(new Diff.Change(Diff.Operation.INSERT, lastequality));
            ...
            if (prevDiff.operation == Diff.Operation.DELETE &&
                ...
                if (overlap_length1 >= overlap_length2) {
                    if (overlap_length1 >= deletion.length() / 2.0 ||
                        ...
                        pointer.previous();
                        pointer.add(new Diff.Change(Diff.Operation.EQUAL,
     ...
     * @param diffs LinkedList of Diff objects.
     */
    void cleanupSemanticLossless(LinkedList<Diff.Change> diffs) {
        ...
        ListIterator<Diff.Change> pointer = diffs.listIterator();
        Diff.Change prevDiff = pointer.hasNext() ? pointer.next() : null;
        Diff.Change thisDiff = pointer.hasNext() ? pointer.next() : null;
        Diff.Change nextDiff = pointer.hasNext() ? pointer.next() : null;
     ...
     * @param diffs LinkedList of Diff objects.
     */
    void cleanupEfficiency(LinkedList<Diff.Change> diffs) {
        if (diffs.isEmpty()) {
            return;
        }
        boolean changes = false;
        Stack<Diff.Change> equalities = new Stack<Diff.Change>();  // Stack of equalities.
        String lastequality = null; // Always equal to equalities.lastElement().text
        ListIterator<Diff.Change> pointer = diffs.listIterator();
        ...
        Diff.Change thisDiff = pointer.next();
        Diff.Change safeDiff = thisDiff;  // The last Diff that is known to be unsplitable.
        while (thisDiff != null) {
            if (thisDiff.operation == Diff.Operation.EQUAL) {
                // Equality found.
                ...
                */
                if (lastequality != null && ((pre_ins && pre_del && post_ins && post_del) ||
                    ...
                    pointer.set(new Diff.Change(Diff.Operation.DELETE, lastequality));
                    // Insert a corresponding an insert.
                    pointer.add(thisDiff = new Diff.Change(Diff.Operation.INSERT, lastequality));
     ...
     * @param diffs LinkedList of Diff objects.
     */
    void cleanupMerge(LinkedList<Diff.Change> diffs) {
        diffs.add(new Diff.Change(Diff.Operation.EQUAL, ""));  // Add a dummy entry at the end.
        ListIterator<Diff.Change> pointer = diffs.listIterator();
        int count_delete = 0;
        int count_insert = 0;
        String text_delete = "";
                ...
                case EQUAL:
                    if (count_delete + count_insert > 1) {
                        ...
                        if (both_types) {
                            ...
                            if (commonlength != 0) {
                                if (pointer.hasPrevious()) {
                                    thisDiff = pointer.previous();
                                    assert thisDiff.operation == Diff.Operation.EQUAL
                        ...
                        if (text_delete.length() != 0) {
                            pointer.add(new Diff.Change(Diff.Operation.DELETE, text_delete));
                        }
                        if (text_insert.length() != 0) {
                            pointer.add(new Diff.Change(Diff.Operation.INSERT, text_insert));
                        }
    ...
    private static long getDeadline(Options options) {
        if (options.getTimeout() > 0) {
