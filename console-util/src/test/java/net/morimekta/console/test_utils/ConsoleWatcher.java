...
/**
 * COPY OF THE <code>ConsoleWatcher</code> test rule in
 * <code>testing-util</code>. Placed here too, so we can
 * use the test library while testing the systems it mocks.
 */
public class ConsoleWatcher extends TestWatcher implements AutoCloseable {
     ...
     */
    public ConsoleWatcher setInput(byte[] in) {
    ...
    protected void failed(Throwable e, Description description) {
        if (dumpOutputOnFailure && outStream.size() > 0) {
            originalErr.println(
                    Color.BOLD + " <<< --- stdout : " + description.getMethodName() + " --- >>>" + Color.CLEAR);
